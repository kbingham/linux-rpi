...
static int vc4_hdmi_audio_init(struct vc4_hdmi *vc4_hdmi)
{
	...
	/*
	 * TODO: Retrieve the address through the dma-ranges mapping
	 */
	addr = of_get_address(dev->of_node, index, NULL, NULL);
	complete_addr = (u64)0x10 << 32;
	complete_addr |= be32_to_cpup(addr);

	vc4_hdmi->audio.dma_data.addr = complete_addr + mai_data->offset;
